namespace P3k.UnityRichConsoleLogger
{
   ...
   public sealed class Logger
   {
      ...
      {
         var callerFile = ParseCallerFileFromStackTrace(stackTrace);
         var resolvedStack = stackTrace;

         if (callerFile == null && TryParseFileFromConditionText(text, out var parsedFile, out var parsedLine))
         {
            callerFile = parsedFile;
            if (string.IsNullOrEmpty(resolvedStack) && parsedLine > 0)
            {
               resolvedStack = $"(at {parsedFile}:{parsedLine})";
            }

            // Strip the redundant file path and severity prefix from the message.
            // Input:  "Assets/.../File.cs(19,50): error CS0246: The type..."
            // Output: "CS0246: The type..."
            var pathIdx = text.IndexOf(parsedFile, StringComparison.Ordinal);
            if (pathIdx >= 0)
            {
               var afterPath = pathIdx + parsedFile.Length;
               var closeParen = text.IndexOf(')', afterPath);
               if (closeParen >= 0)
               {
                  text = text.Substring(closeParen + 1).TrimStart();
               ...
               {
                  text = text.Substring(afterPath).TrimStart();
               }
         ...
         Emit(text, colorHex, callerFile, type, false, resolvedStack);
      }
      ...
      private static string CaptureUserStackFrame()
      {
         ...
         foreach (var f in frames)
         {
            ...
            {
               return $"{fullType}.{methodName} (at {file}:{line})";
            }
         }

         if (firstValidPath != null && firstValidLine > 0)
         {
            return $"{firstValidMethod} (at {firstValidPath}:{firstValidLine})";
         }
      ...
      /// <summary>
      ///    Extracts a file path and line number from a Unity compilation-error message.
      ///    Matches patterns like <c>Assets/Scripts/File.cs(19,50): error CS0246</c>.
      /// </summary>
      /// <param name="text">The log condition text.</param>
      /// <param name="filePath">The extracted file path, or <see langword="null" /> if not found.</param>
      /// <param name="line">The extracted line number, or <c>0</c> if not found.</param>
      /// <returns><see langword="true" /> if a file reference was found; otherwise, <see langword="false" />.</returns>
      private static bool TryParseFileFromConditionText(string text, out string filePath, out int line)
      {
         filePath = null;
         line = 0;
         ...
         // Match path ending in .cs followed by (line,col)
         ...
         var candidate = text.Substring(0, parenIdx).Trim();
         ...
         // Extract line number from (line,col) or (line)
         var closeIdx = text.IndexOf(')', parenIdx);
         if (closeIdx > parenIdx + 1)
         {
            var inner = text.Substring(parenIdx + 1, closeIdx - parenIdx - 1);
            var commaIdx = inner.IndexOf(',');
            var linePart = commaIdx > 0 ? inner.Substring(0, commaIdx) : inner;
            int.TryParse(linePart.Trim(), out line);
         }
